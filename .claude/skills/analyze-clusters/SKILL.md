...
with open('TARGET_FILE') as f:
    raw = json.load(f)
d = raw['clusters'] if isinstance(raw, dict) else raw
...
with open('BEFORE_FILE') as f:
    raw = json.load(f)
before = raw['clusters'] if isinstance(raw, dict) else raw
with open('AFTER_FILE') as f:
    raw = json.load(f)
after = raw['clusters'] if isinstance(raw, dict) else raw
def stats(d):
    total = sum(e['size'] for e in d)
    multi = [e for e in d if e['size'] >= 2]
    noise = [e for e in d if e['size'] == 1]
