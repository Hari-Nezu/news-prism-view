...
## ルール
- 必ずJSON形式のみで回答する（説明文・コードブロック不要）
- category と subcategory は英語IDを使用する
- confidence は 0.0〜1.0 で回答する
- 入力の番号（index）をそのまま results の index に使用する

出力フォーマット: {"results": [{"index": 0, "category": "politics", "subcategory": "domestic_politics", "confidence": 0.95}, ...]}`, taxonomy.BuildClassificationGuide())
