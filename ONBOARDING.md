...
| テーブル | 用途 |
|:--|:--|
| `Article` | 分析済みニュース記事（`embedding: vector(768)`） |
| `FeedGroup` | RSSインクリメンタルグループ（centroid embedding） |
| `FeedGroupItem` | FeedGroup内の記事（重複スキップ） |
| `CompareSession` | 比較検索セッション（キーワード + JSONグループ） |
| `CompareGroupRecord` | セッション内グループ化ニュースイベント |
| `CompareResult` | 比較セッション内の個別分析結果 |
| `YouTubeVideo` | 動画分析結果（字幕タイプ記録） |

`embedding` カラムは `Unsupported("vector(768)")` で定義されており、Prisma の通常CRUDでは操作不可。生SQLが必要（後述）。
...
### RSSフィード（`src/lib/feed-configs.ts`）

デフォルト有効フィード（9個）: NHK政治・国際・経済、朝日、産経、東洋経済、ハフポスト、Google News（政治・経済）
