...
| LLM | ローカル LLM サーバー（llama.cpp 互換, port 8081）— デフォルト `gemma-4-E2B-it-Q8_0` |
| 埋め込み | `multilingual-e5-large-instruct-q8_0`（1024次元） |
...
# LLM / Embed エンドポイント（llama.cpp 互換サーバー想定）
LLM_BASE_URL=http://127.0.0.1:8081
EMBED_BASE_URL=http://127.0.0.1:8081
REFINE_BASE_URL=http://127.0.0.1:8082

# モデル名
LLM_MODEL=gemma-4-E2B-it-Q8_0
CLASSIFY_MODEL=gemma-4-E2B-it-Q8_0
REFINE_MODEL=gemma-4-E2B-it-Q8_0
EMBED_MODEL=multilingual-e5-large-instruct-q8_0

# Embed prefix（e5 系前提のデフォルト。モデルを変える場合は両方上書きすること）
EMBED_DOCUMENT_PREFIX="Instruct: ニュース記事を具体的な出来事・事件単位でクラスタリングする\nQuery: "
EMBED_QUERY_PREFIX="query: "

# クラスタリング閾値
GROUP_CLUSTER_THRESHOLD=0.91
EMBED_CLASSIFY_THRESHOLD=0.8
...
MULTI_MODELS=gemma-4-E2B-it-Q8_0
```

> Ollama（`:11434`）で動かす場合は `*_BASE_URL` と各 `*_MODEL` を Ollama 上のモデル名（例 `gemma3:12b`, `nomic-embed-text`）に揃える。ただし `nomic-embed-text` は 768 次元なので `vector(1024)` カラムに入らない点に注意（後述）。
...
| テーブル | 管理 | 用途 |
|:--|:--|:--|
| `Article` | Prisma | 分析済みニュース記事（`embedding: vector(1024)`） |
...
| `RssArticle` | Prisma | Goバッチ収集記事（`embedding: vector(1024)`） |
...
| `rss_article_embeddings_eval` | Go migration (005) | embedding モデル比較用ベクトル保存（本番 embedding には影響しない） |
| `rss_clustering_eval_runs` | Go migration (005) | eval バッチの実行記録 |
| `rss_clustering_eval` | Go migration (005) | run × model ごとのクラスタ割当結果 |

> migration 004 で `Article` / `FeedGroup` / `RssArticle` / `CompareGroupRecord` / `YouTubeVideo` / `CompareResult` の embedding が **全て `vector(1024)`** に変更され、既存ベクトルは drop されている。`nomic-embed-text`（768次元）等の旧モデルをそのまま使うと dim 不一致で INSERT が失敗する。
...
### embedding モデル評価（オフライン eval バッチ）

```
go run ./batch/cmd/newsprism-batch eval -limit 500 -thresholds e5=0.91,gemma=0.84,ruri=0.90
  → 直近3日の rss_articles から N 件取得
  → e5 / gemma / ruri 各モデルで埋め込み → クラスタリング
  → rss_clustering_eval_runs / rss_clustering_eval に保存
  → 本番 rss_articles.embedding には触らない
```

詳細は `batch/EVAL_BATCH.md`、品質比較は `/compare-eval-models` スキル参照。
...
| ファイル | Route | 説明 |
|:--|:--|:--|
| `app/(internal)/page.tsx` | `/` | 記事URL/タイトル入力 → 分析 → スコア表示 |
| `app/(public)/ranking/page.tsx` | `/ranking` | バッチ収集ニュースのグループランキング表示 |
| `app/(internal)/inspect/page.tsx` | `/inspect` | グループ品質点検（FeedGroup/Snapshot の2タブ） |
| `app/(internal)/compare/page.tsx` | `/compare` | キーワード検索 → グループ化 → 媒体別分析比較 |
| `app/(internal)/youtube/page.tsx` | `/youtube` | YouTubeチャンネル選択 → 字幕分析 |

> route group `(public)` / `(internal)` は URL に影響しない（[`public-private-separation.md`](design/public-private-separation.md) 参照）。
...
デフォルトは **llama.cpp 互換サーバー**（OpenAI 互換 API, port 8081）を前提にしている。Ollama でも動くが、その場合は §3 のとおり全 `*_BASE_URL` / `*_MODEL` を上書きする。

### 必要なモデル（llama.cpp デフォルト）

| 役割 | モデル | 次元/サイズ |
|:--|:--|:--|
| 分析・分類・リファイン | `gemma-4-E2B-it-Q8_0` | — |
| 埋め込み | `multilingual-e5-large-instruct-q8_0` | 1024 |

### 参考：Ollama に切り替える場合の例

```bash
ollama pull gemma3:12b
ollama pull nomic-embed-text   # 768次元なので vector(1024) には入らない
```

Ollama を使う場合は **1024次元の embedding モデル**（例 `bge-m3` など）を選ぶか、migration を独自に調整する。
...
### YouTubeチャンネル（`src/lib/config/youtube-channel-configs.ts`）
...
### pgvector（`Unsupported("vector(1024)")` ※旧 768/310 から移行済み）
